...
func TestServer_handleRepoLookup(t *testing.T) {
	...
	t.Run("not found", func(t *testing.T) {
		...
		if got, want := repoLookupResult(t, "github.com/a/b"), (protocol.RepoLookupResult{}); !reflect.DeepEqual(
			got,
			want,
		) {
...
func TestServer_RepoLookup(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			src: repos.NewFakeSource(&awsSource, nil, awsCodeCommitRepository),
